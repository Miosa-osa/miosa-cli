  ...
  workflows: [
    {
      ...
      title: "Store Provider Key And Bind It To A Sandbox Agent",
      goal: "Let Claude Code, Codex, OpenAI SDKs, Anthropic SDKs, or AI SDK harnesses run inside a sandbox without raw provider keys in prompts, files, or local machines.",
      status: "beta",
      steps: [
        {
          command:
          ...
          purpose:
            "Store the provider API key encrypted as a MIOSA Connect connector. The raw key must not appear in command output.",
          json: true,
        },
        {
          command:
        ...
        },
        {
          ...
          purpose:
            "Bind the connector to the sandbox as a brokered env placeholder.",
          json: true,
        },
        {
          command:
        ...
        {
          command:
            "miosa sandbox prompt <sandbox-id> --provider claude --connector anthropic/workspace-claude --preflight --cwd /workspace --json -- \"Build the requested page and run the tests\"",
          purpose:
            "Preflight the connector, then run the agent CLI inside the sandbox workspace.",
          json: true,
        },
      ],
      success_signals: [
        "connectors create returns id/name/preview without raw provider key",
